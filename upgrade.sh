...
${CURRENT_USER} ALL=(ALL) NOPASSWD: /usr/local/bin/kl-restart
${CURRENT_USER} ALL=(ALL) NOPASSWD: /usr/bin/sed -i *
SUDOEOF
sudo chmod 440 "${SUDOERS_FILE}"
ok "Sudoers rules updated."

# Step 5.1: Install the cgroup-escaping restart helper.
#   The app can't trigger its own restart directly — systemd kills the entire
#   kineticlull cgroup during a restart, including any Popen'd subprocess.
#   This helper uses systemd-run to launch the restart in a new transient unit
#   with its own cgroup, so it survives the restart it's triggering.
KL_RESTART_HELPER="/usr/local/bin/kl-restart"
cat <<'HELPEOF' | sudo tee "${KL_RESTART_HELPER}" > /dev/null
#!/bin/bash
# Generated by KineticLull upgrade.sh — do not edit by hand.
set -e
exec /usr/bin/systemd-run --no-block --collect --unit=kl-restart-$(date +%s%N) \
    /bin/bash -c 'sleep 2 && /usr/bin/systemctl restart kineticlull && sleep 3 && /usr/bin/systemctl restart nginx'
HELPEOF
sudo chmod 755 "${KL_RESTART_HELPER}"
sudo chown root:root "${KL_RESTART_HELPER}"
ok "Restart helper installed at ${KL_RESTART_HELPER}."
